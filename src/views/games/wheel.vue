    ...
    <div class="game-tool">
      <div>
        ...
        <div class="input-control">
          <img width="30" height="20" src="@/assets/coin/JB.webp" />
      ...
      <div>
        <span>Segment</span>
        <div class="segment">
          <van-tabs class="segmentTabs" type="card" @click-tab="changeSegment" color="#31343b" background="#222328"
            title-active-color="#FFFCF3" title-inactive-color="#91a1b5">
            <van-tab title="Low" />
      ...
      <div>
        <span>Row</span>
        <div class="row">
          <van-tabs class="rowTabs" type="card" v-model="rowVal" @click-tab="changeRow" color="#31343b"
...
onMounted(async () => {
  ...
  //这个绑定ws，服务端的消息，会根据gameid，把该游戏的消息传到这里来处理，不属于该游戏的消息不会到这里处理。
  // ws 源码 "@/views/games/service/ws";
...
const onBet = _ => {
  //发送游戏开始请求，等服务端返回游戏结果，在ws消息里开始游戏。
  bet = io.send({
    msgId: 5700,
    data: {
...
const changeSegmentByTitle = (title: string) => {
  if (title == "Low") {
    segment.value = 1;
...
const changeSegment = ({ title }) => {
  changeSegmentByTitle(title);
  game.emit("set:segment", (segment.value - 1));
};

const changeRowByTitle = (title: string) => {
  if (title == "10") {
    row.value = 10;
  } else if (title == "20") {
    row.value = 20;
...
const changeRow = ({ title }) => {
  rowVal.value = title;
  changeRowByTitle(title);
  game.emit("set:row", (row.value / 10 - 1));
};
