...
onMounted(async () => {
  ...
  io.bindEvent(name, (msgId, data) => {
    switch (msgId) {
      case 5070:
        console.log(`【进入游戏】issue:${data.issue},rate:${data.rate}`);

        break;

      case 5403:
        console.log(`【押注回应】`, data);
        game.emit("game:play", Number(data.random), !!data.win);
...
const onBet = _ => {
  //押注设置
  // const result = Math.random()*100|0, isWin = true;
  // agent.emit( "game:play" , result, isWin );
