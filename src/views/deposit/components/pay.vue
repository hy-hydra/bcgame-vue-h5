<template>
    <div class="px-10px">
        ...
        <div class="flex justify-end items-center w-100%">
            <p
                class="m-0 w-54% text-#3cc017 border-b-1px border-b-solid border-b-#3CC017 cursor-pointer hover='bg-#67c23a'">
                {{ $t("how_deposit") }}</p>
        </div>
        <p class="text-14px">{{ props.activeItem.prize }}</p>
        <div class="px-15px py-14px border-rd-8px flex justify-between items-center bg-#17181B">
            <div class="text-#ffff text-15px">ARS</div>
            <input type="text" v-model="inputValue" class="text-15px bg-transparent b-none text-#ffff w-40%">
            <!-- <van-field  class="text-15px bg-transparent b-none text-#ffff w-40%" v-model="inputValue" label="" placeholder="" /> -->
            <div class="text-#3cc017 text-15px w-60%">Extra +ARS{{ Number(inputValue) * 180 }}</div>
        </div>
