  ...
  <div class="bg" @click="pageClick">
    ...
    <div class="header0" v-if="!show">
      ...
      <div class="sweep-header">
        <div class="left">
          <div class="box">
            <img src="../../assets/sweep/jb.png" alt="" @click="sendMsgs" />
            <span>22</span>
          </div>
        </div>
        <div class="center">
      ...
      <div class="scroll">
        ...
        <div>
          <div class="ul" ref="ulx">
            <span v-for="item in bsNum" :key="item">{{ item }}x</span>
            <b class="poup">
    ...
    <!-- <button @click="handleStart">
      <template v-if="isSuccess">win</template>
      <template v-else-if="isFail">fail</template>
      <template v-else="isFail">click</template>
    </button> -->
    <div class="b00">
      <div>
        ...
        <span class="huan huan1"
          >{{ $t("sweep.win") }}:<span>{{ win }}</span></span
        >
      </div>
    </div>
    ...
    <div class="b0">
      <div class="b1" @click="betClick">
        ...
        <div>{{ curNum }}</div>
        <div class="butP" v-if="clickType === 'bet'">
          <div class="betPoup">
            <div
              @click="(e) => betNumClick(e, 1000)"
              :class="{ active: curNum === 1000 }"
            >
              1000
            </div>
            <div
              @click="(e) => betNumClick(e, 100)"
              :class="{ active: curNum === 100 }"
            >
              100
            </div>
            <div
              @click="(e) => betNumClick(e, 8)"
              :class="{ active: curNum === 8 }"
            >
              8
            </div>
            <div
              @click="(e) => betNumClick(e, 800)"
              :class="{ active: curNum === 800 }"
            >
              800
            </div>
            <div
              @click="(e) => betNumClick(e, 50)"
              :class="{ active: curNum === 50 }"
            >
              50
            </div>
            <div
              @click="(e) => betNumClick(e, 5)"
              :class="{ active: curNum === 5 }"
            >
              5
            </div>
            <div
              @click="(e) => betNumClick(e, 500)"
              :class="{ active: curNum === 500 }"
            >
              500
            </div>
            <div
              @click="(e) => betNumClick(e, 30)"
              :class="{ active: curNum === 30 }"
            >
              30
            </div>
            <div
              @click="(e) => betNumClick(e, 3)"
              :class="{ active: curNum === 3 }"
            >
              3
            </div>
            <div
              @click="(e) => betNumClick(e, 300)"
              :class="{ active: curNum === 300 }"
            >
              300
            </div>
            <div
              @click="(e) => betNumClick(e, 20)"
              :class="{ active: curNum === 20 }"
            >
              20
            </div>
            <div
              @click="(e) => betNumClick(e, 2)"
              :class="{ active: curNum === 2 }"
            >
              2
            </div>
            <div
              @click="(e) => betNumClick(e, 200)"
              :class="{ active: curNum === 200 }"
            >
              200
            </div>
            <div
              @click="(e) => betNumClick(e, 10)"
              :class="{ active: curNum === 10 }"
            >
              10
            </div>
            <div
              @click="(e) => betNumClick(e, 1)"
              :class="{ active: curNum === 1 }"
            >
              1
            </div>
          </div>
      ...
      <div class="b1" @click="minesClick">
        ...
        <div>{{ bomNumber }}</div>
        <div class="butP minesP" v-if="clickType === 'mines'">
      ...
      <div class="b100" @click="cashoutClick" v-if="!handShow">
        ...
        >
          <div class="autoverlay">
            <div class="header">
              <img src="../../assets/sweep/h.png" alt="" />
              <img
                src="../../assets/sweep/automatic.png"
                alt=""
                class="letter"
              />
            </div>
            <div style="position: relative; width: 100%">
              ...
              <van-grid
                ...
                "
              >
                <van-grid-item v-for="(value, index) in 25" :key="index">
                  <img
                    src="../../assets/sweep/block.png"
                    v-if="index !== 12"
                    style="width: 100%; height: 100%"
                  />
                  <img
                    src="../../assets/sweep/purple.png"
                    v-else
                    style="width: 100%; height: 100%"
                  />
                </van-grid-item>
            ...
            >
              <van-cell :border="false" title-class="titleClass">
                <template #title>
                  <div style="padding-left: 30px">
                    Number of mines in a single game
                  </div>
                </template>
                <template #value>
                  <van-stepper
                    ...
                    input-width="80"
                    style="
                      --van-stepper-background: #0f1930;
                      --van-stepper-input-text-color: #bbfaff;
                    "
                  />
                </template>
              </van-cell>
              <van-cell
                ...
                :class="{ 'cell-hover': active.includes('1') }"
              >
                <template #title>
                  <van-checkbox
                    name="1"
                    ...
                    style="
                      --van-checkbox-label-color: #73b1e1;
                      --van-checkbox-border-color: transparent;
                    "
                    >Total Round</van-checkbox
                  >
                  <!-- <van-radio-group v-model="checked.checked1" shape="square" style="--van-radio-label-color: #73b1e1; --van-radio-border-color: transparent">
                    <van-checkbox name="1">Total Round</van-checkbox>
                  </van-radio-group> -->
                </template>
                <template #value>
                  <van-stepper
                ...
                <template #extra>
                  <div class="tipB">
                    <div @click="numClick(50)">50</div>
                    <div @click="numClick(100)">100</div>
                    <div @click="numClick(200)">200</div>
                    <div @click="numClick(500)">500</div>
                    <div @click="numClick(999)">999</div>
                  </div>
                </template>
              </van-cell>
              <van-cell
                :border="false"
                title-class="titleClass"
                :class="{ 'cell-hover': active.includes('2') }"
              >
                <template #title>
                  <van-checkbox
                    name="1"
                    ...
                    style="
                      ...
                      --van-checkbox-border-color: transparent;
                    "
                    >Until total money is below</van-checkbox
                  >
                  <!-- <van-radio-group v-model="checked.checked2" shape="square" style="--van-radio-label-color: #73b1e1; --van-radio-border-color: transparent">
                    <van-checkbox name="1" shape="square">Until total money is below</van-checkbox>
                  </van-radio-group> -->
                </template>
                <template #value>
                  <van-stepper
              ...
              <van-cell
                :border="false"
                title-class="titleClass"
                :class="{ 'cell-hover': active.includes('3') }"
              >
                <template #title>
                  <van-checkbox
                    name="1"
                    ...
                    style="
                      ...
                      --van-checkbox-border-color: transparent;
                    "
                    >Unitl total money is above</van-checkbox
                  >
                  <!-- <van-radio-group v-model="checked.checked3" shape="square" style="--van-radio-label-color: #73b1e1; --van-radio-border-color: transparent">
                    <van-checkbox name="1" shape="square">Unitl total money is above</van-checkbox>
                  </van-radio-group> -->
                </template>
                <template #value>
                  <van-stepper
            ...
            <div class="buttons">
              ...
              <div class="button startB">START</div>
            </div>
          </div>
        </van-overlay>
      </div>
...
import { MessageProtoApi, sendMsg } from "@/utils/websocket";
import { sendSock } from "../chat/composables/websocket";
...
const bsNum = ref([
  1.1, 1.26, 1.45, 1.68, 1.96, 2.3, 2.73, 3.28, 3.98, 4.7, 5.88, 7.48, 9.72,
  12.96, 17.83, 25.46, 38.2, 61.11, 106.85, 216.7, 534.75, 2139,
]);
const handleStart = (event) => {
  ...
  grid.value = arr.map((item, index) => {
    ...
    return { isBomb: !!item, count: count.value, isOpen: false };
  });
...
function onItemOpen(item, index) {
  ...
  if (grid.value[index].isBomb) {
    ...
    win.value = 0;
    onItemOpenAll(item, index);
    return stopGame();
  }
  opened.value += 1;
  win.value = curNum.value * bsNum.value[opened.value];
...
const animationNum = () => {
  // 定义初始值和目标值
  const initialValue = 410000; // 起始金额为 0
  const targetValue = 9000000000; // 目标金额为 1000
  ...
  new TWEEN.Tween({ value: initialValue })
    .to({ value: targetValue }, 100000000000) // 设置过渡时长为 2000ms
    .onUpdate((obj) => {
      ...
      // console.log(`当前金额：${currentAmount.value}`);
    })
    .start();
...
const clickType = ref("");
const curNum = ref(200);
const minesData = ref([
  24, 18, 12, 6, 23, 17, 11, 5, 22, 16, 10, 4, 21, 15, 9, 3, 20, 14, 8, 2, 19,
  13, 7, 1,
]);
...
const loadingFunc = () => {
  // 定义初始值和目标值
  const initialValue = 1; // 起始金额为 0
  const targetValue = 100; // 目标金额为 1000
  ...
  // 创建 tween 对象
  new TWEEN.Tween({ value: initialValue })
    .to({ value: targetValue }, 50) // 设置过渡时长为 2000ms
    .onUpdate((obj) => {
...
const startClick = () => {
  ...
  balance.value = balance.value - curNum.value;
  setTimeout(() => {
    STARTNUM.value = true;
    handleStart({});
  }, 550);
...
const sendMsgs = (data) => {
  ...
  //   gameId: 1
  // })
  sendMsg('{"msgId":2000,"data":{"gameId":10001}}');
};
